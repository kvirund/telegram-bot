...
def load_yaml_config() -> YamlConfig:
    ...
    try:
        ...
        ai = AISettings(
            ...
            groq=GroqSettings(
                api_key=groq_data.get("api_key", ""), model=groq_data.get("model", "llama-3.2-90b-text-preview")
            ),
            openrouter=OpenRouterSettings(
                api_key=openrouter_data.get("api_key", ""), model=openrouter_data.get("model", "openai/gpt-4o-mini")
            ),
            local=LocalSettings(
        ...
        system_prompts = SystemPromptsConfig(
            joke_generation=sp_data.get(
                "joke_generation", "You are a witty AI assistant that can generate jokes and humorous content."
            ),
            ...
            mention_response=sp_data.get(
                "mention_response", "You are a straightforward, sarcastic bot. Respond directly to mentions."
            ),
        )
